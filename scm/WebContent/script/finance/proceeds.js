...
function pageSearch(payType){
	...
	//3==预付款到发货
		...
		//清空table
		$(".c tr:gt(0)").remove();
		$.post("/scm/finance/proceedsPageServlet",{"payType":payType,"goPage":1},function(data){
			//如果返回结果为空则不会执行以下代码
			//新增table内容
			for(var i=0;i<data.length-1;i++){
				$(".c").append("<tr>" +
						"<td align='center'><a href='javascript:void(0)' onclick='showItem(this)'>"+data[i].soId+"</a></td>" +
...
function goPage(goPage,payType){
	...
	$(".c tr:gt(0)").remove();
		...
		$.get("/scm/finance/proceedsPageServlet","&goPage="+goPage+"&payType="+payType,function(data){
			//新增table内容
			for(var i=0;i<data.length-1;i++){
				$(".c").append("<tr>" +
						"<td align='center'><a href='javascript:void(0)' onclick='showItem(this)'>"+data[i].soId+"</a></td>" +
...
function checkin(obj){
	if(confirm("确定登记么")){
		var soId=$(obj).parents("tr").find("td:eq(0)").text();
		//servlet登记
		$.get("/scm/finance/proceedsCheckinServlet",{"soId":soId},function(data){
			if(data=="success"){
				alert("登记成功");
				goPage('start','0');
			}else{
...
function showItem(obj){
	var soId=$(obj).text();
	//向服务器发送请求，并返回详细信息
	$.get("/scm/finance/soItemServlet",{"soId":soId},function(data){
		//清空明细表
		$("#headTable td:odd").text("");
		var item=$("#headTable td:odd");
		$(item[0]).text(data.soId);
